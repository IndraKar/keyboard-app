...
import { createMatch, join, start, clampLobbySize, MAX_PLAYERS, MIN_PLAYERS } from "./match.js";
...
export function createMatchmaker({ now = () => Date.now() } = {}) {
  const queues = new Map();   // level -> [{userId, displayName, target, since}]
  const matches = new Map();  // id -> match
  const byCode = new Map();   // joinCode -> matchId

  function enqueue(userId, displayName, level, requestedSize = DEFAULT_TARGET) {
    ...
    queues.set(level, q);
    return tryForm(level);
  }

  function leaveQueue(userId, level) {
    const q = queues.get(level);
    if (!q) return false;
  ...
  function tryForm(level) {
    ...
    const match = createMatch({ id: nextId(), level, now: now() });
  ...
  function createPrivate(userId, displayName, level, rng = Math.random) {
    let code = makeCode(rng);
    while (byCode.has(code)) code = makeCode(rng);
    const match = createMatch({ id: nextId(), level, lobbyKind: "private", joinCode: code, now: now() });
  ...
  return {
    ...
    queueLength: (level) => (queues.get(level) ?? []).length,
    all: () => [...matches.values()],
  };
}
